...
import { getWidgetConfigByType } from './widget-config-store';
import { WidgetConfigMeta } from './widget-config-meta';

@Directive({
    selector: '[appConfigSlot]'
})
export class ConfigSlotDirective implements OnChanges, OnInit, OnDestroy, AfterViewInit {
    ...
    private init() {
        ...
        const wcm = getWidgetConfigByType(this.data.type);
        if (!wcm) {
        ...
        try {
            ...
            const factory = this.componentFactoryResolver.resolveComponentFactory(this.widgetConfigMeta.component);
